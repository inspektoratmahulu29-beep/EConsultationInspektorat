...
export async function onRequest(context) {
  ...
  try {
    if (path === '/api/auth' && request.method === 'POST') {
      ...
      if (acc && acc.password === password) return new Response(JSON.stringify({ status: 'success', role: acc.role }), { headers });
    ...
    if (path === '/api/submit' && request.method === 'POST') {
      ...
      let masalah = rawMasalah.map(text => ({ text: text, jawaban: { auditor: "", ppupd: "", irban1: "", irban2: "", irban3: "", inspektur: "" }, status: "Belum Dijawab" }));
      ...
      let record = {
        ...
        tujuan: tujuan, status: statusAwal, irban1: { status: "", catatan: "" }, irban2: { status: "", catatan: "" }, irban3: { status: "", catatan: "" }, survey: null
      };
      ...
      try {
        if (env.RESEND_API_KEY && env.EMAIL_TO) {
          await fetch('https://api.resend.com/emails', {
            ...
            body: JSON.stringify({
              from: 'E-Consultation <onboarding@resend.dev>', to: [env.EMAIL_TO],
              subject: `Konsultasi Baru: ${record.noForm}`,
              html: `<p>Ada konsultasi baru masuk:</p><ul><li><b>Nama:</b> ${record.nama}</li><li><b>Instansi:</b> ${record.instansi}</li><li><b>Jabatan:</b> ${record.jabatan}</li><li><b>Ditujukan:</b> ${record.tujuan}</li><li><b>No HP:</b> ${record.hp}</li><li><b>Masalah:</b> ${record.masalah.map(m => m.text).join(', ')}</li></ul>`
            })
          });
        }
      } catch (e) {}
    ...
    if (path === '/api/survey' && request.method === 'POST') {
      ...
      let db = await getDB(env);
      for (let i = 0; i < db.records.length; i++) if (db.records[i].id == surveyData.id) { db.records[i].survey = surveyData; break; }
      await setDB(env, db);
    ...
    // Route Verifikasi - PASTIKAN ID JADI ANGKA
    if (path === '/api/verify' && request.method === 'POST') {
      ...
      if (typeof record.masalah[idx] === 'string') record.masalah[idx] = { text: record.masalah[idx], jawaban: { auditor: "", ppupd: "", irban1: "", irban2: "", irban3: "", inspektur: "" }, status: "Belum Dijawab" };
      if (!record.masalah[idx].jawaban) record.masalah[idx].jawaban = { auditor: "", ppupd: "", irban1: "", irban2: "", irban3: "", inspektur: "" };

      if (actionType === 'auditor_verify') record.masalah[idx].jawaban.auditor = jawaban;
      else if (actionType === 'ppupd_review') record.masalah[idx].jawaban.ppupd = jawaban;
      else if (actionType === 'irban_decision') { let irbanNum = (role === 'Irban I') ? 1 : (role === 'Irban II') ? 2 : 3; record.masalah[idx].jawaban['irban' + irbanNum] = jawaban; }
      else if (actionType === 'inspektur_decision') record.masalah[idx].jawaban.inspektur = jawaban;
    ...
    // Route Finalisasi - PASTIKAN ID JADI ANGKA
    if (path === '/api/complete' && request.method === 'POST') {
      ...
      } else if (actionType === 'inspektur_decision' && role === 'Inspektur') record.status = (keputusan === 'setuju') ? 'Disetujui Inspektur' : 'Ditolak Inspektur';
